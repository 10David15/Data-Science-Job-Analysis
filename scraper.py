# CREDITS: scraper modified from source below according to use case: 
...
def get_jobs(job_name, region_id, num_jobs, verbose, path, slp_time):
    ...
    while (len(jobs) < num_jobs and len(jobs) <= max_jobs):
        #let page load - based on internet speed
        #or, wait until webpage is loaded, instead of hardcoding
        time.sleep(slp_time)
        #Test for remove "Sign Up" prompt
        try:
        ...
        try:
            driver.find_element_by_css_selector('[alt="Close"]').click() #clicking the 'X' to close
        except NoSuchElementException:
            pass
        
        #Iterate - each job in page, find jl element to ID job posting
        ...
        try:
            for job_button in job_buttons:  
                ...
                except NoSuchElementException:  #Some postings do not have "Company" tab
                    headquarters = -1
                    size = -1
                    founded = -1
                    type_of_ownership = -1
    ...
    return pd.DataFrame(jobs)  #dict to dataframe for analysis
